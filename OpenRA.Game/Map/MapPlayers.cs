...
namespace OpenRA
{
	public class MapPlayers
	{
		...
		{
			// Dictionary removal and reinsertion can change enumeration order.
			// Keep autogenerated multiplayer slots in their canonical numeric order.
			return Players.OrderBy(p => p.Key.StartsWith("Multi", StringComparison.Ordinal)
				&& Exts.TryParseInt32Invariant(p.Key.AsSpan(5), out var index) ? index : -1)
				.Select(p => new MiniYamlNode($"PlayerReference@{p.Key}",
